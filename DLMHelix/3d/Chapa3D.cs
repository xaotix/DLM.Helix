...
namespace DLM.helix
{
    ...
    public class Chapa3d
    {
        ...
        {
            this.Espessura = Espessura;
            this.Pontos.AddRange(face.Linhas.Select(x => x.P1.Clonar()));
            this.Aberturas.AddRange(face.Furacoes.GroupBy(x=> x.ToString()).Select(x=>x.First()).ToList().FindAll(x => x.GetMinX() > face.MinX && x.GetMaxX() < face.MaxX).Select(y => new DLM.helix.Abertura3d(y.Diametro, y.Origem.X, y.Origem.Y, y.Oblongo, y.Angulo)));
